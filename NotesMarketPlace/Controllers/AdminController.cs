...
namespace NotesMarketPlace.Controllers
{
    public class AdminController : Controller
    {
        ...
        private void SendEmail(string receiver, string subject, string body)
        {
            ...
            {
                var sender = Session["defaultEmail"].ToString();
        ...
        public ActionResult Dashboard(int? SelectedMonth, string sortOrder, string currentFilter, string searchString, int? page)
        {
            try
            {
                usrid = Convert.ToInt32(Session["id"]);
                ...
                if (usrid != 0 && Session["Userrole"] != null)
                {
                    ...
                    ViewBag.User = context.UserRoles.Where(x => x.CreatedDate >= dateCriteria && x.Description.ToLower() == "user").Count();
        ...
        public ActionResult Members(string sortOrder, string currentFilter, string searchString, int? page)
        {
            ...
            {
                //For Table
                List<User> userlist = context.Users.Where(x => x.IsActive == true &&
                                  context.UserRoles.Where(y => y.ID == x.ID).Select(y => y.Description).FirstOrDefault() == "User").ToList();
        ...
        //Manage Type, Add Type, edit Country
        ...
        public ActionResult ManageAdministrator(string sortOrder, string currentFilter, string searchString, int? page)
        {
            ...
            if (usrid != 0 && Session["Userrole"] != null)
            {
                ...
                ViewBag.CurrentFilter = searchString;
                List<User> userlist = context.Users.ToList();
                List<UserRole> userlist1 = context.UserRoles.ToList();
                var process = from l in userlist1.Where(x => x.Description.ToLower() == "admin") 
                              join u in userlist on l.ID equals u.ID into table1
                              from u in table1.DefaultIfEmpty()
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (ModelState.IsValid && model.id == 0)
                    {
                        ...
                        if (repeat == null)
                        {
                            ...
                            context.SaveChanges();
                            var newusrid = context.UserRoles.OrderByDescending(u => u.ID).Select(x => x.ID).FirstOrDefault();
                            ...
                            User usr = new User
                            {
                                ...
                                EmailID = model.email,
                                Password = model.firstname + newusrid.ToString(),
           ...
           try{ 
            ...
            var process = from r in review
                          ...
                          select new AdminNotedetail {
                              ...
                              reviewerphoto = us.ProfilePicture,
                              comment = r.Comments,
